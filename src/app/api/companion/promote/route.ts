...
const promoteCompanionSchema = z.object({
  companion: z.record(z.string(), z.unknown()),
  moduleId: z.string().min(1).max(100),
  adventureId: z.string().min(1).max(100),
});
...
export async function POST(request: Request) {
  try {
    ...
    const parsed = promoteCompanionSchema.safeParse(await request.json());
    if (!parsed.success) {
      return apiError("Invalid request body", 400);
    }
    const { companion: companionData, moduleId, adventureId } = parsed.data;
    const companion = companionData as unknown as Companion;
