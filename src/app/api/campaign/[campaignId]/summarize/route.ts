...
const SUMMARIZE_SYSTEM_PROMPT = `You are a session recorder for a tabletop RPG campaign. Given a session's chat log between a player and their GM, write a concise session summary for the GM's records.
...
export async function POST(
  ...
  try {
    ...
    if (!chatLog) {
      return apiError("No content to summarize", 400);
    }
    ...
    if (!summary) {
      return apiError("Summary generation failed");
    }
