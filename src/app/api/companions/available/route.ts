...
export async function GET(request: Request) {
  try {
    ...
    // --- Former companions ---
    // Fetch all campaigns for this character so we can scan worldState for companions
    ...
    // Collect companions from completed/abandoned campaigns, deduplicated by name
    ...
    const formerCompanions = Array.from(companionMap.values()).map(({ companion, lastSeenIn }) => ({
      companion,
      lastSeenIn,
    }));

    // --- Available roster characters ---
    // All characters for this user except the selected one
    const allChars = await db
      .select()
      .from(characters)
      .where(ne(characters.id, characterId));
