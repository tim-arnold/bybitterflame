    ...
    <div className="bg-stone-900 border border-stone-700 rounded-lg p-3">
      ...
      >
        ...
        <span className="flex items-center gap-2">
          {entries.length > 0 && (
            <span className="text-xs text-stone-400 font-mono" aria-label={`${entries.length} entries`}>{entries.length}</span>
          )}
      ...
      {isOpen && (
        <div id="journal-entries" className="mt-2 space-y-1">
            ...
            return (
              <div key={entry.id} className="border border-stone-800 rounded">
                ...
                <button
                  ...
                  className="w-full flex items-center justify-between px-2 py-1.5 text-left cursor-pointer"
                >
                    ...
                    {isEditing ? (
                      ...
                      <div className="px-2 pb-2">
                        ...
                        <div className="flex items-center justify-between mt-2">
                          ...
                          {isConfirmingDelete ? (
                            <span className="flex items-center gap-2">
                              <span className="text-[10px] text-stone-400">Delete?</span>
                              <button
                              ...
                              <button
                                ...
                                className="text-[10px] text-stone-400 hover:text-stone-300 transition-colors cursor-pointer"
                              >
                                No
                              </button>
                            </span>
                            ...
                            <span className="flex items-center gap-3">
                              ...
                              <button
                                ...
                                className="text-[10px] text-stone-600 hover:text-red-400 transition-colors cursor-pointer"
                              >
                                Delete
                              </button>
                            </span>
          ...
          {formMode === "add" ? (
            ...
            <button
              onClick={openAddForm}
              className="w-full text-left text-xs text-stone-600 hover:text-stone-400 transition-colors pt-1 cursor-pointer"
            >
              + Add entry
            </button>
          )}
    ...
    <div className="p-2 space-y-2">
      ...
      <div className="flex gap-2">
        <button
          onClick={onSave}
          disabled={!canSave}
          className="flex-1 bg-stone-700 hover:bg-stone-600 disabled:opacity-40 disabled:cursor-not-allowed cursor-pointer text-stone-200 text-xs rounded py-1 transition-colors"
        >
          {saveLabel}
        </button>
        <button
          onClick={onCancel}
          className="flex-1 text-stone-400 hover:text-stone-300 text-xs transition-colors cursor-pointer"
        >
          Cancel
        </button>
      </div>
