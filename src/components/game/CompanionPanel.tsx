...
const STATUS_COLORS: Record<string, string> = {
  ...
  departed: "bg-stone-800 text-stone-400 border-stone-700",
  dead: "bg-stone-800 text-stone-400 border-stone-700",
};
  ...
  return (
    <div className="border border-stone-700 rounded-md overflow-hidden">
      ...
      >
        ...
        <span aria-hidden="true" className="text-stone-400 text-xs">{expanded ? "▲" : "▼"}</span>
      </button>
      ...
      {expanded && (
        <div className="px-3 py-2 space-y-2 bg-stone-900/50 text-xs text-stone-300">
          ...
          {companion.level < 10 && (
            <div>
              XP: <span className="text-stone-100">{companion.xp ?? 0}/{companion.level * 10}</span>
              <span className="text-stone-500 ml-1">to next level</span>
            </div>
          )}
          ...
          {(companion.gold ?? 0) > 0 && (
            <div>
              Gold: <span className="text-[var(--color-gold)]">{companion.gold} gp</span>
            </div>
          )}
          ...
          <div className="grid grid-cols-3 gap-1 font-mono text-center">
            {(["str", "dex", "con", "int", "wis", "cha"] as const).map((stat) => (
              <div key={stat} className="bg-stone-800 rounded px-1 py-0.5">
                <div className="text-stone-400 text-[10px] uppercase">{stat}</div>
          ...
          {(companion.equipment?.length ?? 0) > 0 && (
            <div className="pt-1 border-t border-stone-700 space-y-0.5">
              <div className="text-stone-400 text-[10px] uppercase tracking-wider mb-1">Equipment</div>
              {companion.equipment.map((item, i) => (
                <div key={i} className="flex items-baseline justify-between gap-2">
                  <span className="text-stone-300">{item.name}</span>
                  {item.damage && (
                    <span className="text-stone-400 font-mono shrink-0">{item.damage}</span>
                  )}
                </div>
              ))}
            </div>
          ...
          {(companion.spells?.length ?? 0) > 0 && (
            <div className="pt-1 border-t border-stone-700 space-y-0.5">
              <div className="text-stone-400 text-[10px] uppercase tracking-wider mb-1">Spells</div>
              {companion.spells.map((spell, i) => (
                <div key={i} className="flex items-baseline justify-between gap-2">
                  <span className="text-stone-300">{spell.name}</span>
                  <span className="text-stone-400 font-mono shrink-0">T{spell.tier}</span>
                </div>
              ))}
            </div>
          )}
          ...
          {companion.personality && (
            <div className="space-y-1 pt-1 border-t border-stone-700">
              ...
              <div className="flex items-center gap-2">
                ...
                <span className="text-stone-400 ml-auto">{companion.personality.loyalty}/10</span>
              </div>
    ...
    <div className="bg-stone-900 border border-stone-700 rounded-lg p-3 space-y-2">
      ...
      >
        ...
        <span className="flex items-center gap-1.5">
          <span className="text-xs text-stone-400 bg-stone-800 px-1.5 py-0.5 rounded-full">
            {visible.length}
          </span>
