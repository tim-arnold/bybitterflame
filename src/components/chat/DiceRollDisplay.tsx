...
function DieFace({ value, sides, delay }: { value: number; sides: number; delay: number }) {
  ...
  if (phase === "waiting") {
    return (
      <span aria-label={`waiting to roll d${sides}`} className="inline-flex items-center justify-center w-14 h-14 text-4xl leading-none rounded-md text-stone-600 bg-stone-800/50">
        {isD6 ? <span className="opacity-30" aria-hidden>{D6_FACES[0]}</span> : <span className="text-xl font-bold opacity-50" aria-hidden>?</span>}
      </span>
    );
  }
  ...
  if (phase === "tumbling") {
    return (
      <span aria-label="rolling" aria-live="off" className="inline-flex items-center justify-center w-14 h-14 text-4xl leading-none rounded-md bg-stone-800 animate-dice-tumble text-stone-300">
        {isD6 ? <span aria-hidden>{D6_FACES[displayValue - 1]}</span> : <span className="text-xl font-bold" aria-hidden>{displayValue}</span>}
      </span>
    );
  }
  ...
  // revealed
  return (
    <span aria-label={`rolled ${value}`} className="inline-flex items-center justify-center w-14 h-14 text-4xl leading-none rounded-md bg-stone-900 transition-all duration-300 scale-110 animate-dice-land text-[var(--color-gold)]">
      {isD6 ? <span aria-hidden>{D6_FACES[value - 1]}</span> : <span className="text-xl font-bold" aria-hidden>{value}</span>}
    </span>
  );
}
...
function SingleRollDisplay({ roll, baseDelay }: { roll: DiceRoll; baseDelay: number }) {
  ...
  const rollSummary = `${roll.name ? roll.name + ": " : ""}rolled ${roll.rolls.join(", ")}, total ${roll.total}`;

  return (
    <div className="flex items-baseline gap-2.5 py-1.5" aria-label={rollSummary}>
      {roll.name && (
        <span className="text-base font-medium text-stone-400 w-28 text-right shrink-0" aria-hidden>
          {roll.name}
        </span>
      )}
...
export function DiceRollDisplay({ rolls, onComplete }: DiceRollDisplayProps) {
  ...
  return (
    <div role="region" aria-label="Dice rolls" className="my-3 bg-stone-900/80 border border-stone-700 rounded-lg px-4 py-3">
      <div className="flex items-center gap-2 mb-2">
        <span className="text-[10px] uppercase tracking-widest text-stone-400" aria-hidden>Rolling dice</span>
      </div>
