...
function getAtmosphericError(error: unknown): string {
  // For Anthropic API errors, extract the human-readable message from the response body
  if (error instanceof Anthropic.APIError) {
    const isTransient = error.status === 529 || error.status >= 500;
    if (!isTransient) {
...
export function createStreamingResponse(
  ...
  messages: Message[],
  apiKey?: string,
  onComplete?: (usage: UsageResult) => void | Promise<void>,
  model: string = MODEL_SONNET,
  ...
  return new ReadableStream({
    async start(controller) {
      try {
        ...
        // Await onComplete so DB writes finish before the stream closes.
        // On Cloudflare Workers the execution context is killed once the
        // response stream ends, so fire-and-forget promises get dropped.
        await onComplete?.({
