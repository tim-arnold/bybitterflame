...
- Local D1 migration applied: `wrangler d1 execute shadowdark --local --file=migrations/0012_cache_tokens.sql`
...
### 2a. Cache tokens appear in API response
...
```bash
# Local
wrangler d1 execute shadowdark --local --file=migrations/0012_cache_tokens.sql

# Verify columns exist
wrangler d1 execute shadowdark --local --command "PRAGMA table_info(user);" | grep cache
```
...
1. Note current values:
   ```bash
   wrangler d1 execute shadowdark --local \
...
- [ ] Verify new columns in production:
  ```bash
  wrangler d1 execute shadowdark --remote \
